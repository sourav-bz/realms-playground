...
const NewMultiTraxnProposal = ({ wallet, connection}:{ wallet: WalletSigner, connection: Connection}) => {
    ...
    const createProposalHandler = async () =>{
        ...
        `NEW give me 0.3, 0.4 sol ${new Date().toTimeString()}`,
        `give me money`,
        ...
        let ins1 = SystemProgram.transfer({
        ...
        let ins2 = SystemProgram.transfer({
            fromPubkey: nativeTreasury,
    ...
    return(
        <button style={{margin: '20px'}} onClick={createProposalHandler}>
            create 0.3, 0.4 sol proposal - multi Txn
        </button>
    )
}

export default NewMultiTraxnProposal;